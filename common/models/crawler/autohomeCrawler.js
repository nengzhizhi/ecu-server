...
AutohomeCrawler.forumUrlReg = /^http:\/\/club.autohome.com.cn\/bbs\/forum-([a-z])-([\d]+)-1.html$/;
...
AutohomeCrawler.crawlForumUrl = function (forumUrl, start, number, callback) {
	...
	if (!forumUrl.match(self.postForumUrlReg))
		return callback(new Error('错误的地址！'));

	var forumUrls = [];

	for (var i = start; i <= number; i ++) {
		forumUrl.replace(self.postForumUrlReg, function ($1, $2, $3) {
	...
	superagent.get(url)
		.end(function (err, sres) {
			...
			for (var i = 1; i <= pageNumber; i ++) {
				url.replace(self.postUrlReg, function ($1, $2, $3, $4) {
